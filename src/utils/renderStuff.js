...
const $projectNote = document.getElementById('project-notes')

// const $propSetOutput = document.getElementById('prop-set-output')
// const $propSetLength = document.getElementById('prop-set-length')

export function renderProjectInfo (projects, projectId) {
  const currentProject = projects.find(project => project.id === projectId)
  const { name, nota } = currentProject

  $vmId.innerHTML = `${projectId}`
  $projectName.innerHTML = `${name}`
  $projectNote.innerHTML = `${nota}`
